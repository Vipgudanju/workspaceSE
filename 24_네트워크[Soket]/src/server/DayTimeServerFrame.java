	...
	public class DayTimeServerThread extends Thread {
		...
		public void run() {
			try {
				...
				while (true) {
					displayLog("1.DayTimeServerThread:클라이언트 연결 요청대기(스레드대기)");
					Socket socket=serverSocket.accept();
					displayLog("2.DayTimeServerThread:클라이언트와 연결된 서버쪽 소캣생성" + socket);
					PrintWriter out = new PrintWriter(new OutputStreamWriter(socket.getOutputStream()));
					displayLog("3.DayTimeServerThread:소캣으로 부터 OutputStream생성");
					Date serverDate = new Date();
					String serverTimeStr = serverDate.toLocaleString();
					out.println("강사 server time:" +serverTimeStr);
					out.flush();
					displayLog("4.DayTimeServerThread:소캣으로 부터 생성된 OutputStream에서 서버시간 쓰기;" + socket);
					socket.close();
					displayLog("5.DayTimeServerThread:클라이언트와 연결된 서버쪽 소캣닫기" + socket);
				}
			}catch (Exception e) {
